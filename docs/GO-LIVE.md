...
- [x] **Delete the diagnostic route** `app/api/test-email/route.ts` (done 2026-06-18).
- [x] **Remove debug logging** — 1 `console.log` in `lib/email.ts` (done 2026-06-18).
...
> **Status (2026-06-18):** `moonscloset.com` **purchased through Vercel** (registrar + host = Vercel), so DNS is auto-configured — no manual A/CNAME records needed. Domain shows **Verified**. Remaining: attach to the `clothing-website` project (if not already), set primary domain + `www` redirect, confirm HTTPS serves the site.
...
1. ~~Resend dashboard → **Domains → Add Domain** `moonscloset.com`.~~ **Done 2026-06-18 — Resend reports the domain Verified ("ready to send emails").**
2. ~~Add the **DKIM + SPF TXT records** Resend gives you to the domain's DNS.~~ Auto-configured via the Vercel-managed DNS.
3. **Remaining:** set env `RESEND_FROM_EMAIL=noreply@moonscloset.com` (or `pesanan@…`) in Vercel, then redeploy.
...
| `RESEND_FROM_EMAIL` | Set to `noreply@moonscloset.com` — Resend domain `moonscloset.com` is verified (2026-06-18). |
